...
export default {
  ...
  activated(){
    //根据货币类型来过滤不同表单
    this.currency = this.$store.state.routerParams.positionData.fiatCode;
  ...
  methods: {
    ...
    submit(){
      let queryForm = {
        countryCode: this.$store.state.routerParams.positionData.alpha2, // 国家Code
        id: this.$store.state.sellForm ? this.$store.state.sellForm.positionData.id : '', // 不传为新增卡信息，传为修改卡信息
        fiatCode: this.$store.state.routerParams.positionData.alpha2, // 法币Code
      };
